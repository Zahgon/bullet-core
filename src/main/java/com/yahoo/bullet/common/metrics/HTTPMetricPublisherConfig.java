...
public class HTTPMetricPublisherConfig extends BulletConfig {
    ...
    static {
        ...
        VALIDATOR.define(GROUP).defaultTo(DEFAULT_GROUP).checkIf(Validator::isString).castTo(Validator::asString);
        VALIDATOR.define(DIMENSIONS).defaultTo(DEFAULT_DIMENSIONS).checkIf(Validator.isMapOfType(String.class, String.class));
        VALIDATOR.define(RETRIES).defaultTo(DEFAULT_RETRIES).checkIf(Validator::isPositiveInt).castTo(Validator::asInt);
        VALIDATOR.define(RETRY_INTERVAL_MS).defaultTo(DEFAULT_RETRY_INTERVAL_MS).checkIf(Validator::isPositiveInt).castTo(Validator::asInt);
        VALIDATOR.define(MAX_CONCURRENCY).defaultTo(DEFAULT_MAX_CONCURRENCY).checkIf(Validator::isPositiveInt).castTo(Validator::asInt);
    }
