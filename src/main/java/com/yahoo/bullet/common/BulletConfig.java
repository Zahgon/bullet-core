...
public class BulletConfig extends Config {
     ...
     */
    public static final List<Map<String, String>> DEFAULT_RESULT_METADATA_METRICS = makeMetadata(ImmutablePair.of(Concept.QUERY_METADATA, "Query"), ImmutablePair.of(Concept.QUERY_ID, "ID"), ImmutablePair.of(Concept.QUERY_STRING, "Query String"), ImmutablePair.of(Concept.INNER_QUERY_METADATA, "Inner Query"), ImmutablePair.of(Concept.QUERY_RECEIVE_TIME, "Receive Time"), ImmutablePair.of(Concept.QUERY_FINISH_TIME, "Finish Time"), ImmutablePair.of(Concept.SKETCH_METADATA, "Sketch"), ImmutablePair.of(Concept.SKETCH_ESTIMATED_RESULT, "Was Estimated"), ImmutablePair.of(Concept.SKETCH_STANDARD_DEVIATIONS, "Standard Deviations"), ImmutablePair.of(Concept.SKETCH_FAMILY, "Family"), ImmutablePair.of(Concept.SKETCH_SIZE, "Size"), ImmutablePair.of(Concept.SKETCH_THETA, "Theta"), ImmutablePair.of(Concept.SKETCH_UNIQUES_ESTIMATE, "Uniques Estimate"), ImmutablePair.of(Concept.SKETCH_MINIMUM_VALUE, "Minimum Value"), ImmutablePair.of(Concept.SKETCH_MAXIMUM_VALUE, "Maximum Value"), ImmutablePair.of(Concept.SKETCH_ITEMS_SEEN, "Items Seen"), ImmutablePair.of(Concept.SKETCH_NORMALIZED_RANK_ERROR, "Normalized Rank Error"), ImmutablePair.of(Concept.SKETCH_MAXIMUM_COUNT_ERROR, "Maximum Count Error"), ImmutablePair.of(Concept.SKETCH_ACTIVE_ITEMS, "Active Items"), ImmutablePair.of(Concept.WINDOW_METADATA, "Window"), ImmutablePair.of(Concept.WINDOW_NAME, "Name"), ImmutablePair.of(Concept.WINDOW_NUMBER, "Number"), ImmutablePair.of(Concept.WINDOW_SIZE, "Size"), ImmutablePair.of(Concept.WINDOW_EMIT_TIME, "Emit Time"), ImmutablePair.of(Concept.WINDOW_EXPECTED_EMIT_TIME, "Expected Emit Time"));
    ...
    static {
        VALIDATOR.define(QUERY_DEFAULT_DURATION).defaultTo(DEFAULT_QUERY_DURATION).checkIf(Validator::isPositive).castTo(Validator::asLong);
        VALIDATOR.define(QUERY_MAX_DURATION).defaultTo(DEFAULT_QUERY_MAX_DURATION).checkIf(Validator::isPositive).castTo(Validator::asLong);
        VALIDATOR.define(AGGREGATION_DEFAULT_SIZE).defaultTo(DEFAULT_AGGREGATION_SIZE).checkIf(Validator::isPositiveInt).castTo(Validator::asInt);
        VALIDATOR.define(AGGREGATION_MAX_SIZE).defaultTo(DEFAULT_AGGREGATION_MAX_SIZE).checkIf(Validator::isPositiveInt).castTo(Validator::asInt);
        VALIDATOR.define(AGGREGATION_COMPOSITE_FIELD_SEPARATOR).defaultTo(DEFAULT_AGGREGATION_COMPOSITE_FIELD_SEPARATOR).checkIf(Validator::isString);
        VALIDATOR.define(RAW_AGGREGATION_MAX_SIZE).defaultTo(DEFAULT_RAW_AGGREGATION_MAX_SIZE).checkIf(Validator::isPositiveInt).castTo(Validator::asInt);
        VALIDATOR.define(COUNT_DISTINCT_AGGREGATION_SKETCH_ENTRIES).defaultTo(DEFAULT_COUNT_DISTINCT_AGGREGATION_SKETCH_ENTRIES).checkIf(Validator::isPowerOfTwo).castTo(Validator::asInt);
        VALIDATOR.define(COUNT_DISTINCT_AGGREGATION_SKETCH_SAMPLING).defaultTo(DEFAULT_COUNT_DISTINCT_AGGREGATION_SKETCH_SAMPLING).checkIf(Validator::isFloat).checkIf(Validator.isInRange(0.0, 1.0)).castTo(Validator::asFloat);
        VALIDATOR.define(COUNT_DISTINCT_AGGREGATION_SKETCH_FAMILY).defaultTo(DEFAULT_COUNT_DISTINCT_AGGREGATION_SKETCH_FAMILY).checkIf(Validator::isString).checkIf(Validator.isIn(ALPHA_SKETCH_FAMILY, QUICKSELECT_SKETCH_FAMILY));
        VALIDATOR.define(COUNT_DISTINCT_AGGREGATION_SKETCH_RESIZE_FACTOR).defaultTo(DEFAULT_COUNT_DISTINCT_AGGREGATION_SKETCH_RESIZE_FACTOR).checkIf(Validator::isPowerOfTwo).checkIf(Validator.isInRange(1, 8)).castTo(Validator::asInt);
        VALIDATOR.define(GROUP_AGGREGATION_SKETCH_ENTRIES).defaultTo(DEFAULT_GROUP_AGGREGATION_SKETCH_ENTRIES).checkIf(Validator::isPowerOfTwo).castTo(Validator::asInt);
        VALIDATOR.define(GROUP_AGGREGATION_MAX_SIZE).defaultTo(DEFAULT_GROUP_AGGREGATION_MAX_SIZE).checkIf(Validator::isPositiveInt).castTo(Validator::asInt);
        VALIDATOR.define(GROUP_AGGREGATION_SKETCH_SAMPLING).defaultTo(DEFAULT_GROUP_AGGREGATION_SKETCH_SAMPLING).checkIf(Validator::isFloat).checkIf(Validator.isInRange(0.0, 1.0)).castTo(Validator::asFloat);
        VALIDATOR.define(GROUP_AGGREGATION_SKETCH_RESIZE_FACTOR).defaultTo(DEFAULT_GROUP_AGGREGATION_SKETCH_RESIZE_FACTOR).checkIf(Validator::isPowerOfTwo).checkIf(Validator.isInRange(1, 8)).castTo(Validator::asInt);
        VALIDATOR.define(DISTRIBUTION_AGGREGATION_SKETCH_ENTRIES).defaultTo(DEFAULT_DISTRIBUTION_AGGREGATION_SKETCH_ENTRIES).checkIf(Validator::isPowerOfTwo).castTo(Validator::asInt);
        VALIDATOR.define(DISTRIBUTION_AGGREGATION_MAX_POINTS).defaultTo(DEFAULT_DISTRIBUTION_AGGREGATION_MAX_POINTS).checkIf(Validator::isPositiveInt).castTo(Validator::asInt);
        VALIDATOR.define(DISTRIBUTION_AGGREGATION_GENERATED_POINTS_ROUNDING).defaultTo(DEFAULT_DISTRIBUTION_AGGREGATION_GENERATED_POINTS_ROUNDING).checkIf(Validator::isPositiveInt).castTo(Validator::asInt);
        VALIDATOR.define(TOP_K_AGGREGATION_SKETCH_ENTRIES).defaultTo(DEFAULT_TOP_K_AGGREGATION_SKETCH_ENTRIES).checkIf(Validator::isPowerOfTwo).castTo(Validator::asInt);
        VALIDATOR.define(TOP_K_AGGREGATION_SKETCH_ERROR_TYPE).defaultTo(DEFAULT_TOP_K_AGGREGATION_SKETCH_ERROR_TYPE).checkIf(Validator::isString);
        VALIDATOR.define(RESULT_METADATA_ENABLE).defaultTo(DEFAULT_RESULT_METADATA_ENABLE).checkIf(Validator::isBoolean);
        ...
        VALIDATOR.define(WINDOW_DISABLE).defaultTo(DEFAULT_WINDOW_DISABLE).checkIf(Validator::isBoolean);
        VALIDATOR.define(WINDOW_MIN_EMIT_EVERY).defaultTo(DEFAULT_WINDOW_MIN_EMIT_EVERY).checkIf(Validator::isPositiveInt).castTo(Validator::asInt);
        VALIDATOR.define(RATE_LIMIT_ENABLE).defaultTo(DEFAULT_RATE_LIMIT_ENABLE).checkIf(Validator::isBoolean);
        VALIDATOR.define(RATE_LIMIT_MAX_EMIT_COUNT).defaultTo(DEFAULT_RATE_LIMIT_MAX_EMIT_COUNT).checkIf(Validator::isPositiveInt).castTo(Validator::asInt);
        VALIDATOR.define(RATE_LIMIT_TIME_INTERVAL).defaultTo(DEFAULT_RATE_LIMIT_TIME_INTERVAL).checkIf(Validator::isPositiveInt).castTo(Validator::asInt);
        VALIDATOR.define(PUBSUB_CONTEXT_NAME).defaultTo(DEFAULT_PUBSUB_CONTEXT_NAME).checkIf(Validator.isIn(Context.QUERY_PROCESSING.name(), Context.QUERY_SUBMISSION.name()));
        ...
        VALIDATOR.define(QUERY_PARTITIONER_ENABLE).defaultTo(DEFAULT_QUERY_PARTITIONER_ENABLE).checkIf(Validator::isBoolean);
        ...
        VALIDATOR.define(EQUALITY_PARTITIONER_FIELDS).checkIf(Validator.isListOfType(String.class)).checkIf(Validator.hasMaximumListSize(MAXIMUM_EQUALITY_FIELDS)).unless(Validator::isNull).orFail();
        VALIDATOR.define(EQUALITY_PARTITIONER_DELIMITER).defaultTo(DEFAULT_EQUALITY_PARTITIONER_DELIMITER).checkIf(Validator::isString);
    ...
    private static Object mapifyMetadata(Object metadata) {
        ...
        for (Map m : entries) {
            mapping.put((String) m.get(RESULT_METADATA_METRICS_CONCEPT_KEY), (String) m.get(RESULT_METADATA_METRICS_NAME_KEY));
        }
        return mapping;
    }
    ...
    private static boolean isMetadata(Object metadata) {
        try {
            for (Map m : (List<Map>) metadata) {
                if (m.size() != 2) {
                    log.warn("Meta should only contain the keys {}, {}. Found {}", RESULT_METADATA_METRICS_CONCEPT_KEY, RESULT_METADATA_METRICS_NAME_KEY, m);
                    return false;
                }
